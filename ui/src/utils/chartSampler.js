...
export default class chartSampler {
  ...
  //}
	...
	static computeNormalCdfSamplesYAxis(min, max, numberOfSamples, mean, std) {
		let steps = ((max - min) / numberOfSamples);
		...
		for (let i = 1; i <= numberOfSamples; i++) {
			...
			let y = this.normalCdf(x, mean, std);
	...
	static computeLogNormalCdfSamplesXAxis(min, max, numberOfSamples, mean, std) {
		let steps = ((max - min) / numberOfSamples);
