...
//        String output = target("/projects").request().header("X-Credential-Username", "test").accept(MediaType.APPLICATION_JSON).get
//        (String.class);
//        JSONArray parsedObject = new JSONArray(output);
//
//        for(int i = 0; i < parsedObject.length(); i++){
//            JSONObject project = new JSONObject(parsedObject.get(i).toString());
//            assertNotNull(project.get("id").toString());
//            assertNotNull(project.get("metadata").toString());
//            assertNotEquals(223, project.get("members").toString().length());
//        }
//    }
    ...
    public void testIngestSpace() throws IOException {
        ...
        final FormDataMultiPart multiPartEntity = new FormDataMultiPart().field("project", jsontext);

        Response response = target("/projects").register(MultiPartWriter.class).request()
    ...
    public void testGrantPrivileges() throws IOException {
        ...
        Response response = target("/projects/" + id + "/grant").register(MultiPartWriter.class).request()
            .header("x-auth-userinfo",
                "{\"sub\":\"\",\"email_verified\":true,\"name\":\"Incore Tester Tester\",\"preferred_username\":\"incrtest\"," +
                    "\"given_name\":\"Incore Tester\",\"family_name\":\"Tester\",\"email\":\"tolbert+incoretester@illinois.edu\"}")
            .header("x-auth-usergroup", "{\"groups\": [\"incore_user\", \"incore_coe\"]}")
