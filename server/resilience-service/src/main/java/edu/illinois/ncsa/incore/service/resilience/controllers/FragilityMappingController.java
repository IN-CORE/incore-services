...
import edu.illinois.ncsa.incore.service.resilience.daos.IFragilityDAO;
import edu.illinois.ncsa.incore.service.resilience.daos.IMappingDAO;
import edu.illinois.ncsa.incore.service.resilience.models.FragilityMappingSet;
...
import edu.illinois.ncsa.incore.service.resilience.models.dto.MappingRequest;
import edu.illinois.ncsa.incore.service.resilience.models.dto.FragilityMappingResponse;
import edu.illinois.ncsa.incore.service.resilience.models.mapping.ResilienceMapper;
import edu.illinois.ncsa.incore.service.resilience.models.mapping.MatchFilterMap;
...
public class FragilityMappingController {
    ...
    @ApiOperation(value = "Gets list of resilience mappings", notes="Apply filters to get the desired set of resilience mappings")
    public List<MappingSet> getMappings(@HeaderParam("X-Credential-Username") String username,
        ...
        if (queryMap.isEmpty()) {
            mappingSets = this.mappingDAO.getMappingSets("resilience");
        } else {
            mappingSets = this.mappingDAO.queryMappingSets(queryMap, "resilience");
        }
        if (!spaceName.equals("")) {
    ...
    @Path("{mappingSetId}")
    @Produces({MediaType.APPLICATION_JSON})
    @ApiOperation(value = "Gets a resilience mapping set by Id", notes="Get a particular resilience mapping set based on the id provided")
    public MappingSet getMappingSetById(@HeaderParam("X-Credential-Username") String username,
                                        @ApiParam(value="hexadecimal resilience mapping id", example = "5b47b2d9337d4a36187c7563") @PathParam("mappingSetId") String id) {
        Optional<MappingSet> mappingSet = this.mappingDAO.getMappingSetById(id, "resilience");
    ...
    @Produces({MediaType.APPLICATION_JSON})
    @ApiOperation(value = "Create a resilience Mapping", notes="Post a resilience mapping set that maps a resilience to an inventory's attributes")
    public MappingSet uploadMapping(@HeaderParam("X-Credential-Username") String username,
                                    @ApiParam(value="json representing the resilience mapping") MappingSet mappingSet) {
    ...
    @Consumes(MediaType.APPLICATION_JSON)
    @Produces({MediaType.APPLICATION_JSON})
    @ApiOperation(value = "Map each inventory to a resilience set Id based on the input mapping Id",
        notes = "Returns a json where key is the inventory id that is mapped to a resilience set id based on the input mapping id")
    public FragilityMappingResponse mapFragilities(@HeaderParam("X-Credential-Username") String username,
        ...
        Optional<MappingSet> mappingSet = this.mappingDAO.getMappingSetById(mappingSetId, "resilience");
        ...
        for (Feature feature : features) {
            String fragilityKey = mapper.getResilienceCurveFor(mappingRequest.mappingSubject.schemaType.toString(),
            ...
            if (ObjectId.isValid(fragilityKey)) {
                ...
                if (queriedFragilitySets.containsKey(fragilityKey)) {
                    ...
                    if (fragilitySet.isPresent()) {
                        ...
                        // if currFagility is set to null for a queried resilience,
                        // it means we already read the resilience and determined that it doesn't have read access.
                        queriedFragilitySets.put(fragilityKey, currFragility);
                    }
                }

                // If we found a matching resilience and user has read access to it
